...
const emptyForm = (): Omit<Medication, "id" | "log"> => ({
  name: "",
  dosage: "",
...
export default function MedicationsPage() {
  ...
  useEffect(() => {
    setMeds(storage.medications.getAll());
  }, []);
  ...
  function save() {
    ...
    storage.medications.save(med);
    if (!editing) {
      storage.activity.push({
        type: "medication",
        label: `Added medication: ${med.name}`,
  ...
  function toggleDose(med: Medication, time: string) {
    ...
    const updated: Medication = {
      ...
      log: {
        ...med.log,
        [today]: { ...todayLog, [time]: !todayLog[time] },
      },
    };
    storage.medications.save(updated);
    setMeds(storage.medications.getAll());
  }
  ...
  const takenDoses = meds.flatMap((m) =>
    m.times.filter((t) => m.log[today]?.[t])
  );
  const progress = allDoses.length > 0
    ? Math.round((takenDoses.length / allDoses.length) * 100)
    : 0;
        ...
        <div className="card">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm text-gray-500">
              {takenDoses.length} of {allDoses.length} taken
            </span>
          </div>
